...
//		    	shoppingcartManager.doNotUseCoupon(sUuid);
		    	this.shoppingcartManager.doNotUsePromo(shoppingcart);
		...
		boolean flag = true; //购物车内的产品是否全是“每日78折”目录下，true表示不是，false表示是
...
//		request.setAttribute("shippingRateList",shippingRateList);
		...
		//购物车促销
		Vector<PRule> prules = this.pruleManager.getShoppingcartPromotionRules();
		request.setAttribute("prules", prules);
        ...
        if (cookie != null)
        {
            ...
            if(result == Coupon.STATE_VALID){
        		...
        		if(cookie!=null){//进入购物车页面时，假如cookie中没有购物车uuid，则初始化一辆购物车给用户
        		    ...
        		    BigDecimal dis_mem = cart.getCartDiscountAmount();	//未输入优惠劵、礼品卡、积分之前的优惠金额
        		    BigDecimal subTotal = cart.getSubtotal();//部分营销后的总额
        		    
        		    shoppingcart = shoppingcartManager.refreshCart(sUuid, request, response);
        		    
        		    if(shoppingcart.getSubtotal().compareTo(subTotal) < 0){	//输入优惠劵后的subTotal是否小于原subTotal
        		    	
        		    	 ajaxView.setData(subTotal.subtract(shoppingcart.getSubtotal()));
            ...
            }else if(result == Coupon.STATE_SAME_PRULE){	//前后优惠劵对应的促销信息相同
            	ajaxView.setStatus((short)5);
            	ajaxView.setMsg("同一优惠劵促销！");
            }else{
            	shoppingcartManager.doNotUseCoupon(cookie.getValue());
            	this.shoppingcartManager.refreshCart(cookie.getValue(), request, response);
	...
	public PRuleManager getPruleManager() {
		return pruleManager;
	}
